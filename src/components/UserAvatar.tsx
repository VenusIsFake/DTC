...
export default function UserAvatar({ name, src, size = 36, className = "" }: UserAvatarProps) {
  ...
  if (src) {
    return (
      <span
        className={`relative inline-block rounded-full overflow-hidden border border-dtc-gold/40 shrink-0 ${className}`}
  ...
  return (
    <span
      aria-hidden="true"
      className={`inline-flex items-center justify-center rounded-full bg-dtc-wash border border-dtc-line/60 text-dtc-gold font-bold shrink-0 ${className}`}
