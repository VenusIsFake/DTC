        ...
        {items.map((item) => {
          ...
          return (
            <button
              ...
              className="glass-card glass-card-hover rounded-xl sm:rounded-lg overflow-hidden border border-dtc-line/40 text-left group focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-dtc-gold"
            >
              <div className="relative aspect-video w-full bg-black">
                {item.poster_url ? (
                  ...
                  <div className="absolute inset-0 flex items-center justify-center bg-dtc-wash/60">
                    <Play className="w-8 h-8 text-dtc-gold fill-current" />
                  </div>
                )}
                <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent flex items-center justify-center">
                  <div className="w-11 h-11 sm:w-14 sm:h-14 rounded-full bg-dtc-gold text-dtc-paper flex items-center justify-center shadow-lg group-hover:scale-110 transition-transform">
              ...
              <div className="p-3.5 sm:p-4 space-y-1">
                {item.speaker && (
                  <span className="text-[11px] sm:text-xs font-semibold text-dtc-gold block truncate">
                    {item.speaker}
                  </span>
                )}
                <h3 className="text-sm sm:text-base font-heading font-bold text-dtc-ink group-hover:text-dtc-gold transition-colors line-clamp-2">
                  {item.title}
                </h3>
                {item.description && (
                  <p className="text-[11px] sm:text-xs text-dtc-inkMuted line-clamp-2">{item.description}</p>
                )}
              </div>
            </button>
          );
          ...
          <div className="relative z-10 w-full max-w-4xl space-y-3">
            <div className="flex items-center justify-between gap-3">
              <div className="min-w-0">
                {active.speaker && (
                  <p className="text-xs font-semibold text-dtc-gold truncate">{active.speaker}</p>
                )}
                <h3 className="text-sm sm:text-lg font-heading font-bold text-dtc-ink truncate">{active.title}</h3>
              </div>
              <button
            ...
            <div className="relative aspect-video w-full rounded-xl sm:rounded-lg overflow-hidden border border-dtc-line/50 bg-black shadow-lg">
              {activeYouTube ? (
                <iframe
                  key={active.id}
            ...
            {active.description && (
              <p className="text-xs sm:text-sm text-dtc-lineDark leading-relaxed px-1">{active.description}</p>
            )}
          </div>
        </div>
      )}
