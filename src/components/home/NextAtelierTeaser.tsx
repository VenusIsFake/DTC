...
export default function NextAtelierTeaser({ atelier }: { atelier: AnnouncementBoardItem }) {
  return (
    <section className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8" aria-live="polite">
      ...
      >
        <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-[10px] sm:text-[11px] font-bold bg-[#755B18] text-[#F7F5F0] shrink-0 self-start sm:self-center">
          <CalendarDays className="w-3.5 h-3.5" />
          <span>Prochain atelier</span>
        </div>
        <div className="flex-1 min-w-0 space-y-0.5">
          <h2 className="text-sm sm:text-lg font-heading font-bold text-[#16233A] truncate group-hover:text-[#755B18] transition-colors">
            {atelier.title}
          </h2>
          <p className="text-[11px] sm:text-xs text-[#5C6672] flex items-center gap-3 flex-wrap">
            {atelier.event_date && (
              <span className="flex items-center gap-1">
                <CalendarDays className="w-3 h-3 text-[#755B18]" />
                {formatDateTime(atelier.event_date)}
              </span>
            )}
            {atelier.location && (
              <span className="flex items-center gap-1">
                <MapPin className="w-3 h-3 text-[#755B18]" />
                {atelier.location}
              </span>
            )}
        ...
        <span className="flex items-center gap-1 text-[11px] sm:text-xs font-semibold text-[#755B18] shrink-0">
