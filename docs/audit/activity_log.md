...
## 2026-09-03 (3) — v2.7 : liens d'invitation à usage unique + changement de mot de passe

**Demande de Venus : « un lien spécial et compliqué à envoyer aux gens, utilisable même mur fermé, clé à usage unique : la personne crée son compte et récupère l'accès bureau » + « une option pour changer son mot de passe ». Push en prod demandé, avec revue par agents juges avant (shutdown PC demandé puis ANNULÉ par Venus).**

- **Migration `20260903_invite_links.sql` appliquée en prod (MCP, `schema.sql` resynchronisé)** : table `invite_links` (token hex 256 bits unique, rôle, expiration 30 j, `used_at/used_by`), RLS activé SANS policies (accès RPC uniquement). 5 RPC SECURITY DEFINER : `create_invite_link` (bureau+, liens admin réservés aux admins), `list_invite_links` (**tokens des liens ADMIN masqués pour les non-admins**), `revoke_invite_link`, `invite_link_status` (accordé à anon pour la page publique ; `coalesce(...,'invalid')`), `redeem_invite_link` (claim atomique FOR UPDATE ; bannis refusés ; mise à jour de rôle protégée par rang SUR la ligne profiles — deux liens concurrents ne peuvent pas rétrograder, compte sans profil ne consomme rien).
- **Page `/invitation/<token>`** : mur + chrome exemptés (comme /candidature), `noindex`, validation serveur du token (regex 64 hex + RPC) — formulaire de création de compte + rachat direct si session (invité→bureau), bouton connexion pour comptes existants, écran « déjà utilisé/expiré/invalide » propre.
- **Console Utilisateurs** : bouton « Lien d'invitation » (rôle choisi ; option admin visible uniquement des admins), liste des liens (badges Disponible/Utilisé par X/Expiré, copie par ligne avec feedback ✓, révocation avec confirm, note « lien admin masqué » pour bureau).
- **Mot de passe** : `PasswordChangeCard` dans /espace → onglet profil — vérification du mot de passe actuel par re-signin puis `auth.updateUser` (évite requires_recent_login).
- **Revue indépendante AVANT application** (Venus : « judge agent » — le juge visuel ne couvre pas le code, donc 2 agents revue code + sécurité) : verdicts SHIP-WITH-NOTES ×2, 0 critique. Fixes pliés avant d'appliquer : HIGH = masquage des tokens admin dans `list_invite_links` (un bureau ne peut plus voler un lien admin en attente), MED = refus des bannis au rachat + feedback de copie par ligne, LOW = coalesce 'invalid', garde de rang sur le UPDATE, boutons d'écran final selon rôle, préfixe SiteChrome resserré, ligne reformattée. C'est la première fois que la revue précède l'application — le flux à garder.
- **Vérifié EN DIRECT** : status « ok:bureau »/« invalid »/« used » ; page 200 token valide + « Lien indisponible » sur jeton invalide, mur 307 /annonces intact ; chaîne complète : inscription → guest → rachat = « bureau » (profil + lien `used_by` vérifiés) → 2e rachat = « déjà utilisé » ; mot de passe : PUT /auth/v1/user 200, nouveau mdp 200 / ancien 400. User + lien de test supprimés (0 restant).
- **Qualité + déploiement :** tsc 0, eslint 0, vitest 22/22, build vert ; push `231228a` + `npm run deploy` = `dtc-b556nex30` Ready 53 s (1er essai : échec réseau « fetch failed » après upload, retry OK), aliasé dtc-fmdc.
- **Question de Venus sur schema.sql :** confirmé — le fichier ne touche jamais la base ; seul `apply_migration` via MCP écrit en prod, et Vercel ne joue aucun SQL (supabase/ exclu de l'upload).

## 2026-09-03 (4) — v2.8 : parcours d'adhésion streamliné (funnel invité → dossier → paiement) + hotfix liens d'invitation

**Demande de Venus : « streamliner » la venue des nouveaux membres — à l'inscription on propose l'adhésion, puis formulaire de profil, puis une page montant à payer + coordonnées bancaires / rendez-vous bureau + bouton WhatsApp vers un numéro défini dans la console. « Max de customisation depuis la console, comme le reste du site. » Signalement en cours de route : « function gen_random_bytes(integer) does not exist » à la création d'un lien d'invitation — fix dans la foulée.**

- **Hotfix liens d'invitation (cause racine) :** `create_invite_link` tournait `security definer set search_path=''` en appelant `gen_random_bytes()` NON qualifié — pgcrypto vit dans le schéma `extensions`, introuvable via search_path vide → crash « does not exist » à chaque création. Correctif : token = 2×`gen_random_uuid()` (pg_catalog, toujours résolvable) concaténés et dé-hyphenés — toujours exactement 64 hex, ~244 bits d'entropie. Vérifié en direct : fonction déployée sans référence à gen_random_bytes, gabarit token = 64 hex.
- **Migration `20260903_membership_flow.sql` appliquée en prod (MCP, `schema.sql` resynchronisé)** : ① colonne `profiles.membership_status` ('none'/'pending'/'member', défaut none) — SANS grant de colonne UPDATE (l'invité ne peut pas s'auto-stamper via REST, seuls les RPC SECURITY DEFINER écrivent) ; ② `submit_membership_request(full_name, promo, phone, bio)` : validations serveur (nom 2–120, promo 2000–2100, regex téléphone `^\+?[0-9][0-9 .-]{6,29}$`, bio ≤1000), écrit profil + status 'pending' en un shot, garde `and role='guest'` SUR le UPDATE (une approbation concurrente ne peut jamais être rétro-stampée) ; ③ `cancel_membership_request()` : l'invité retire sa demande lui-même (pending→none) ; ④ `approve_guest` approuve → role member + membership_status 'member' ; ⑤ `bureau_list_profiles`/`admin_list_profiles` renvoient membership_status (badge console). Anon révoqué partout, authenticated accordé.
- **Parcours invité (`/espace`, remplace l'écran d'attente quand le toggle est actif)** — `MembershipFlow.tsx` : ① carte d'invitation « Devenir membre » (texte console, montant cotisation en tease) → ② formulaire profil (nom, promo via promo_years, téléphone, bio) → ③ « Dossier envoyé » + panneau paiement : montant affiché, coordonnées bancaires (lignes sélectionnables), section « en main propre », bouton WhatsApp vert (`wa.me/<numéro>?text=<message pré-rempli>`, `noopener noreferrer`). Modifier le dossier / retirer la demande disponibles.
- **Console — nouvel onglet « Adhésion » (bureau+)** : toggle marche/arrêt du parcours, texte d'invitation, montant cotisation (texte libre), toggles + textes virement/en main propre, numéro WhatsApp (normalisé chiffres seuls + message pré-rempli + lien d'aperçu testable), message post-envoi. Tout vit dans `site_settings` clés `membership_*` — même mécanique que le reste du site.
- **Onglet Utilisateurs :** badge vert « Dossier d'adhésion reçu » + compteur — le bureau voit qui a soumis, vérifie le paiement, puis Approuver (flux inchangé).
- **Revue indépendante AVANT application (flux v2.7 reconduit)** : agents code + sécurité, verdicts SHIP ×2, 0 critique. Fixes pliés avant d'appliquer : garde `role='guest'` sur le UPDATE (TOCTOU approbation concurrente, sécurité-LOW), per-key typeof dans `getMembershipSettings` (robustesse types). Notés pour plus tard (non bloquants) : pin du WITH CHECK profiles_self_update sur membership_status (défense en profondeur, la protection tient déjà via le grant de colonnes), statut potentiellement obsolète après rétrogradation admin_set_role (cosmétique — tout gate de données lit `role`).
- **Vérifié EN DIRECT (chaîne complète, compte jetable)** : inscription → guest ✓ ; submit 204 → profil `membership_status='pending'` + promo/téléphone/nom écrits ✓ ; téléphone invalide rejeté (« Numéro de téléphone invalide ») ✓ ; anon exec RPC = 42501 ✓ ; cancel 204 → none ✓ ; resubmit 204 ✓ ; user test supprimé (0 restant). `membership_enabled` laissé OFF — Venus l'active depuis la console après avoir rempli RIB/WhatsApp (le parcours s'affiche alors aux invités ; la mécanique RPC est déjà vivante).
- **Exposition notée (acceptée, même classe que marquee/sponsors) :** `site_settings` est anon-lisible via REST (policy `settings_public_read` existante) → RIB/numéro WhatsApp lisibles sans compte. RIB = info de crédit publiable par nature ; à garder en tête si un jour le bureau y met des données perso dans « en main propre ».
- **Qualité + déploiement :** tsc 0, eslint 0, vitest 22/22, build vert ; `npm run deploy` = `dtc-lnfjzd4os` Ready 44 s, aliasé dtc-fmdc (fraîcheur via `vercel ls`) ; smoke : mur 307 /espace→/candidature 200 suivi, /admin 307, aucun 500.
- **graphify-out : toujours périmé** (CLI query-only, skill absent) — reporté comme les sessions précédentes.

## 2026-09-03 (5) — audit propreté mobile + fixes (cartes Utilisateurs, cibles tactiles, fade onglets console)

**Demande de Venus : « verify the cleanliness of the front end on mobile view, create a report and standby for the go to start fixing » — audit d'abord, rapport, puis « go » pour les fixes.**

- **Audit (17 vues, prod, 390×844 émulé touch, admin temp via API admin GoTrue)** : 13 pages/onglets + lightbox galerie, modal vidéo TEDx, menu burger, console, cibles tactiles. 16/17 propres — zéro overflow horizontal, aucune erreur console, overlays conformes iOS (body lock fixed via useOverlayDialog). Rapport : `docs/audit/2026-09-03_mobile-cleanliness-report.md`. Faux positifs écartés : « grandes bandes vides » = Reveal.tsx (IntersectionObserver) jamais déclenché en capture pleine page ; « tuile noire » galerie = artefact lazy-load (image 200/115 Ko). Compte admin temp créé via `POST /auth/v1/admin/users` (l'INSERT SQL direct dans auth.users 500 chez GoTrue — pas de ligne auth.identities), supprimé après vérif (0 restant, profil en cascade).
- **Fix 1 — Utilisateurs console, colonne texte écrasée (HIGH)** : à 390 px, nom/email/« Inscrit » se réduisaient à ~7 px (un caractère par ligne, les 4 cartes) — `min-w-0 flex-1` laissait la colonne s'écraser au lieu de faire passer les actions à la ligne. Correctif `UsersTab.tsx` : `flex-1 min-w-[150px]` → actions wrappées sur leur propre ligne. Mesuré avant/après : 7 px → 283 px.
- **Fix 2 — cibles tactiles < 24 px (WCAG 2.5.8)** : CTAs inline HomeContent (×4, py-2 -my-2 — zéro décalage visuel) + liens nav footer (py-1.5). Avant : 17 éléments < 24 px ; après : 0.
- **Fix 3 — bande d'onglets console sans signal de scroll** : fade dégradé au bord droit (masque statique masqué à l'arrivée en bout — ref + onScroll). Vérifié : monté quand strip déborde (scrollW 1100 > client 366), disparaît au bout (scrollLeft 734 = atEnd).
- **Vérification :** tsc 0, eslint 0, vitest 22/22, build vert. Le juge visuel d'après-fix est resté bloqué (bug harnais session : Read image → upload CDN sans pixels, analyze_image refuse les URLs signées) — validation par mesures DOM locales ET prod à la place (largeurs mesurées, overflow, fade, cibles). Captures : /tmp/dtc-mobile/fx*.png.
- **Déploiement :** push `30fe35e` + `npm run deploy` = `dtc-4nb9bcuxr` Ready 50 s (fraîcheur `vercel ls` : 58 s), aliasé dtc-fmdc. Re-vérifié EN DIRECT sur prod : cartes 283 px, fade monté, 0 cible < 24 px, pas d'overflow.
- **Note multi-agents :** v2.8 a atterri en parallèle pendant l'audit (l'arbre portait son WIP types.ts + migration — jamais touché, commit différé jusqu'à leur fusion).

## 2026-09-09 (1) — fix console Mandats aveugle derrière le mur (v2.8.1) + purge doublon

**Demande de Venus : « Mandats & organigrammes, this, when i add a new mandat nothing shows up, clear the duplicate ones i added, and investigate and fix, then push to prod » — le nouvel onglet console Mandats semble ignorer ses créations ; nettoyage des doublons, cause racine, fix, prod.**

- **Symptôme reconstitué depuis la base :** deux lignes créées à 27 s d'écart (« Mandat 2026–2027 » puis « Mandat: 2026-2027 » — libellé différent donc la contrainte d'unicité ne bloquait pas le 2e essai), toutes deux sans membres ni infographie. Les INSERT passaient, le rechargement de la liste renvoyait vide → « rien ne s'affiche ».
- **Cause racine :** le verrou mur v2.5 (`20260901_wall_content_lock.sql`) a gated les policies SELECT `mandates_public_read`/`mandate_members_public_read` sur `site_is_open()` seul — pour {anon, authenticated}, donc AUSSI bureau/admin connectés. Toutes les autres tables console (annonces, podcast, TEDx, pages/objets événement, galerie, sections À propos) ont reçu le bypass `or is_bureau_or_admin()` dans la même migration ; mandates/mandate_members ont été oubliées. Mur fermé = lecture console vide, INSERT (gate rôle seul) toujours permis → créations silencieuses invisibles. `idea_board` (vue droits propriétaire) immunise l'onglet Idées — pas d'autre table console touchée.
- **Fix — migration `20260909_mandates_bureau_read.sql` appliquée en prod (MCP) :** les deux policies passent à `((select public.site_is_open()) or public.is_bureau_or_admin())` — exactement le gabarit des tables sœurs. `schema.sql` resynchronisé (section v2.8.1). Revue sécurité AVANT application (flux v2.7/v2.8 reconduit) : PASS, zéro élargissement — ALTER POLICY ne touche pas au périmètre de rôles (reste `to anon, authenticated`), anon et membres non-bureau restent à 0 ligne mur fermé, noms/photos de `mandate_members` toujours cachés pré-lancement.
- **Purge doublon :** DELETE de « Mandat: 2026-2027 » (essai avec deux-points) ; conservé « Mandat 2026–2027 » (côté 19:39:28, style du libellé existant). Aucun membre rattaché aux deux lignes, FK en cascade sans effet. État final : 2 mandats (2025–2026 Courant, 2026–2027), 12 membres intacts.
- **Vérifié EN DIRECT (impersonation de rôles, mur fermé) :** bureau/admin → 2 mandats + 12 membres (la liste console affichera donc bien les deux) ; membre authentifié sans profil bureau → 0 ; anon → 0. Advisors post-DDL : rien de nouveau (WARNs = classe role-helpers anon-exec acceptée + fuites mdp/MFA côté Venus, INFO invite_links RPC-only voulu).
- **Prod :** fix 100 % base de données — déjà vivant à l'instant de l'apply_migration (le site lit la base en direct). Zéro changement de code applicatif → pas de `npm run deploy` nécessaire (rebuild identique) ; commit migration + schema.sql + journal poussé sur GitHub.

## 2026-09-09 (2) — audit complet « reste de l'app » (4 agents) + hotfix mandat + v2.9 fixes

**Demande de Venus : « audit the rest of the app, and make sure its perfect », interrompue en cours de route par « record "new" has no field "updated_at", when trying to definir courant for a new mandat » (hotfix prioritaire), puis reprise et livraison complète.**

- **Hotfix « Définir courant » (v2.8.2a, prod + `71f7f07`) :** le trigger baseline `touch_updated_at` (fonction `set_updated_at()` écrit `NEW.updated_at`) avait été créé sur `mandates` avec 6 tables de contenu — mais `mandates` n'a PAS de colonne `updated_at`. Tout UPDATE sur la table plantait ; l'UPDATE n'avait jamais été tenté avant (« courant » était fixé à la création). Trigger droppé en prod (migration `drop_mandates_touch_updated_at`), vérifié par UPDATE bureau roulé back ; array du DO-block schema.sql amputé de 'mandates'.
- **Audit 4 agents parallèles (code-correctivité, sécurité, DB vivante, a11y statique) + gates :** tsc 0, eslint 0, vitest 22/22, npm audit 2 HIGH → **0** (js-yaml maxTotalMergeKeys + sharp/libheif, fixes in-range).
- **Sécurité : CLEAN sur toute la surface v2.7→v2.8.1** (tokens invitation 244 bits, redeem atomique FOR UPDATE, masquage tokens admin, wall-bypass /invitation exactement préfixé, PII profiles scopée par column-grants, 4 routes API re-vérifiées, service_role server-only, storage mime/size sur INSERT+UPDATE). Nouveau lint advisor « authenticated ×23 SECURITY DEFINER » = même classe acceptée (garde interne vérifiée dans les 14 RPC privilégiés).
- **DB — tautologie critique corrigée (v2.8.2b) :** la policy `applications_open_insert` testait `p.recruitment_id = recruitment_id` dans la sous-requête `recruitment_positions p` → toujours VRAI (référence non qualifiée liée à `p`, pas à la ligne candidate) : un candidat pouvait apparier un poste d'UNE AUTRE campagne à la campagne ouverte. Fix appliqué (ledger `applications_position_campaign_fix` + `applications_policy_qualification_fix`) : seule la comparaison ambigüe est qualifiée `applications.recruitment_id` — la 1re tentative (tout qualifié) cassait TOUS les inserts anon (portée vide en WITH CHECK) — **rattrapé par la sonde live avant tout vrai candidat**. Prouvé par REST réel : appariement valide 201, poste d'une autre campagne 42501. + 2 index FK invite_links (perf). Leçons : sonde INSERT = colonnes obligatoires complètes (study_year/motivation check) ; déparse pg_policies qualifie toujours — ne pas s'y fier.
- **Code P2 :** double-clic possible sur « Nouvelle campagne »/postes RecruitmentsTab (pas de garde saving) → gardes `creating`/`savingPosition` + boutons disabled ; PromoYearsEditor montrait « Enregistré ✓ » même si l'upsert échouait (erreur jetée) → erreur affichée, plus dédoublonnage + validation liste vide.
- **Code P3 :** erreurs de mutation console silencieuses (Annonces/Idées/Galerie/Podcast/Commissions/Candidatures — session expirée = clic sans effet) → bannières role="alert" partout ; chargements en échec masqués en « vide » (Aucun talk/Galerie vide/Soyez le premier à commenter) → erreur conservée ; youtube_id console validé via `parseYouTubeId` (plus d'erreur PostgREST anglaise) ; uploads écrasant le formulaire avec un snapshot périmé pendant l'envoi (EventsTab/AboutTab) → setState fonctionnel ; compteurs d'onglets du hub /annonces figés au SSR → callbacks `onCountChange` feeds→hub ; réordonnancement membres mandat no-op sur doublons de sort → rénumber complet par upsert (normalise les doublons à chaque move), sort insert = max+1.
- **Accessibilité (vers AA) :** contrastes hero événement + bande TEDx sur photos sombres (#16233A sur #0B132B ≈ 1.2:1 !) → palette claire conditionnelle ; skip-link « Aller au contenu » + main#contenu ; text-red-600 (4.24:1) → red-700 (5.7:1) sur ~40 occurrences, icône YouTube re-#FF0000 ; focus ring inputs plein et 60 % (form.tsx, AuthModal, IdeaComments) ; aria-pressed onglets hub + filtres galerie ; animate-pulse neutralisé prefers-reduced-motion ; mot de passe temporaire/lien invitation/dossier d'adhésion/« Candidature envoyée » annoncés (role=status, focus sur le titre) ; label manquant #rec-share-url ; hiérarchie titres (h3 console/espace→h2, podcast h1→h2→h3) ; alts organigrammes descriptifs ; pan clavier (flèches) sur zoom organigramme + recadrage avatar (SC 2.5.7) ; aria-labels d'icônes répétés disambiguïsés par titre d'élément. Reste accepté : drag-only pinch-zoom mobile (alternative molette/curseur), spinners sans role.
- **Qualité + déploiement :** tsc 0, eslint 0, vitest 22/22, build vert ; `npm run deploy` (fraîcheur via `vercel ls`) + push ; re-smoke prod (mur 307, /candidature 200, anon REST applications []).

## 2026-09-10 (1) — audit « classe des profils liés » (sync avatars/noms) + parité annonces (v2.9.1)

**Suite au fix avatar mandats, demande de Venus : « audit the rest of these similar features for perfect stability ». Périmètre = toute surface joignant ou copiant des données de profil (nom, avatar) : vues board, RPCs, annuaire, embeddings client.**

- **2 agents parallèles (DB + client), verdict : classe STABLE, 0 P0/P1/P2 ailleurs — 1 vraie anomalie de parité + 1 régression de mon propre fix de la veille.**
- **Parité avatars annonces (le trou) :** `idea_board`/`comment_board` exposent `author_avatar`, `announcement_board` NON → les auteurs d'annonces rendaient des initiales à jamais (aussi bien côté public SSR que bureau). Fix : colonne `author_avatar` ajoutée en bout de vue (CREATE OR REPLACE VIEW ne peut qu'appendre, jamais réordonner — leçon mémoire respectée), embed bureau étendu à `avatar_url` (hint FK `!announcements_author_id_fkey` conservé), rendu bascule sur `UserAvatar` (dernier renderer d'initiales artisanal du repo éliminé). Migration `20260910_announcement_author_avatar.sql` appliquée en prod + schema.sql resynchronisé (**au passage : le def 933 de schema.sql avait perdu `poster_url` — drift corrigé**).
- **Régression v2.9 corrigée :** mon fix de la veille copiait l'avatar du compte lié dans `photo_url` au pick/save → copie snapshot qui GÈLE la photo (elle gagne sur l'embed live, plus jamais de sync si le membre change d'avatar). Revenu à la bonne sémantique : `photo_url` = photo MANUELLE uniquement, l'avatar du compte lié vit dans l'embed fallback et suit les changements. Sonde prod : 0 ligne affectée par l'ancien comportement (Venus n'avait pas re-sauvegardé).
- **Hygiène :** `announcement_attendees` exclut désormais les membres bannis (parité `member_directory`, RPC bureau-only) ; vérifié aussi : aucune FK ambiguë vers profiles (1 seule par table, hint déjà en place sur announcements), toutes les vues = security_invoker + LEFT JOIN (mur fermé = noms NULL, jamais de lignes perdues), fallbacks UI propres partout (annuaire, participants, UsersTab, IdeaComments/IdeasBoard).
- **Qualité + déploiement :** tsc 0, eslint 0, build vert ; migration MCP-applied et vue vérifiée en prod (information_schema) ; `npm run deploy` = `dtc-l8g0lrpy8` Ready 51 s, push `9301e6d`.
...
## 2026-09-11 (2) — nouveau logo officiel + host backstage tué, page secrète relocalisée (v2.9.3)

**Venus : nouveau logo (IMG_0173.PNG = émail or sur marine, nettoyé) à archiver + déployer ; et « just change the goddamn dentalkclub-fmdc.vercel.app, i dont want it to work no more ».**

- **Logo :** original archivé `assets/branding/originals/IMG_0173_2026-09-11.PNG` + master recadré `assets/branding/dentalk-club-emblem-master.png` (bbox auto sur fond marine) ; dérivés régénérés : `public/logo.png` 716, `icon-512` 512, `apple-touch-icon` 180, `src/app/icon.png` 192, `favicon.{png,ico}` multi-16/32/48, `og-image.jpg` 1200×630 (emblème centré sur marine). Remplace la identité lettrine Auguste partout (navbar, JSON-LD, OG/Twitter, PWA, favicon).
- **Host `dentalkclub-fmdc.vercel.app` = MORT :** retiré du projet via `DELETE /v9/projects/dtc/domains/…` (a marché pour un domaine vercel.app historique) + garde middleware → 404 nu. Le hostname ne sert plus rien.
- **Page secrète relocalisée sur sous-domaine illisible de NOTRE zone : `vx72kq9.dentalkclubfmdc.com`** (CNAME → `cname.vercel-dns.com`) — URL non devinable, ne porte plus le nom du club.
- **Chromeless réel :** leçon — `usePathname` (client) voit l'URL navigateur PRÉ-rewrite, jamais le chemin réécrit par le middleware → la garde `/secret` de SiteChrome ne pouvait pas feu sur le host backstage. Fix : le middleware pose `x-secret-page: 1` sur la requête, `layout.tsx` (server, `await headers()` — Next 15 async) lit et passe `forceChromeless` à SiteChrome → navbar+footer absents du rendu.
- **Page illisible v2 :** tout texte réel éliminé (titre compris) — soupe de glyphes (tofu, runique, zálgo, flou + jitter CSS), `aria-label` français seul pour lecteurs d'écran, `prefers-reduced-motion` respecté. Juge : pass.
