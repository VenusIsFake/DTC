...
create table if not exists public.profiles (
  ...
  role text not null default 'guest' check (role in ('guest', 'member', 'bureau', 'admin')),
...
-- Public signup stays open; fresh accounts get role 'guest' (no annuaire, no
-- ideas/votes/RSVP writes) until a bureau/admin approves them from the
-- console (approve_guest). Mirrors migration 20260903_guest_role.sql.
-- ============================================================================
...
-- Active member = validated + not banned (guests are observers only).
create or replace function public.is_active_member()
returns boolean
language sql stable security definer set search_path = ''
as $$
...
-- Member-tier profile reads: validated members only (guests fall back to the
-- anon-tier policy, which the site wall already gates).
drop policy if exists "profiles_member_read" on public.profiles;
create policy "profiles_member_read" on public.profiles
  for select to authenticated
  using ((select public.is_active_member()));

-- Bureau view gains role/ban/created so the Users tab can surface pending guests.
...
-- approve=true → guest becomes member; approve=false → account deleted.
...
-- ============================================================================
-- v2.7 (2026-09-03) — one-time invitation links. Mirrors migration
-- 20260903_invite_links.sql. Wall-exempt /invitation/<token> page lets the
-- recipient create an account and receive the link's role (default bureau).
-- ============================================================================
...
-- Deliberately NO policies: RPC-only access (guards live inside each fn).
...
-- Public status check for the wall-exempt page ('ok:<role>'/'used'/'expired'/
-- 'invalid'). Anon-granted on purpose: the token itself is the secret.
...
-- Atomic one-time redemption. Grants the link's role when the caller ranks
-- below it; returns 'already' (NOT consumed) when they hold equal access.
...
-- ============================================================================
-- v2.8 (2026-09-03) — membership funnel + invite-link hotfix.
-- Mirrors migration 20260903_membership_flow.sql.
-- ============================================================================

-- Hotfix: gen_random_bytes lives in the pgcrypto `extensions` schema and is
-- unresolvable from an empty search_path — mint tokens from core
-- gen_random_uuid() instead (same 64-hex shape).
create or replace function public.create_invite_link(new_role text default 'bureau')
returns text
language plpgsql security definer set search_path = ''
as $$
...
-- Funnel state on guest profiles.
alter table public.profiles
...
-- Guest submits (or resubmits) their membership form: server-side validation,
-- profile fields + status written in one shot.
...
-- Pending guest can retract their form themselves.
...
-- Approval stamps the funnel status too.
create or replace function public.approve_guest(target_id uuid, approve boolean default true)
returns void
language plpgsql security definer set search_path = ''
as $$
...
-- Console views expose the funnel state.
drop function if exists public.bureau_list_profiles();
create function public.bureau_list_profiles()
