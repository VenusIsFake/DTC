...
* **Backoffice:** `/admin` console open to **bureau + admin** roles — Utilisateurs (admin-only : rôles/bannissements/invitations avec mot de passe temporaire, réinitialisation, suppression), **Accueil** (marquee, slogan, stats, partenaires — `site_settings`), Annonces (+ « Notifier par email » via Resend), Idées (modération), **Podcast Studio** (import YouTube par URL + upload de poster), Événements (visibilité de section, CRUD TEDx + pages `/events/[slug]`, uploads d'images, éléments éditables), **Galerie** (CRUD + upload), À propos (sections éditables, mandats avec **photos de membres, comptes liés, import de l'équipe précédente, réordonnancement**), Commissions & listes, **Candidatures** (campagne d'appel à candidatures bureau — titre/intro/postes/open-close — + gestion des candidatures reçues : statut, suppression, export CSV). Badges explicites quand une clé serveur (YouTube/Resend/service_role) manque. Detail: `docs/platform/production-readiness.md`.
