...
* **Backoffice:** `/admin` console open to **bureau + admin** roles — 8 onglets, l'onglet actif persiste dans l'URL (`/admin?tab=…`) : Utilisateurs (admin-only : rôles/bannissements/invitations avec mot de passe temporaire, réinitialisation, suppression), **Accueil** (marquee, slogan, partenaires, **bascule « Accès public du site »** — `site_settings`; le bandeau stats de l'accueil a été retiré, clé `home_stats` dormante), Annonces & Idées (pills internes : annonces + « Notifier par email » via Resend · modération des idées), **Podcast Studio** (import YouTube par URL + upload de poster), Événements (visibilité de section, CRUD TEDx + pages `/events/[slug]`, uploads d'images, éléments éditables), **Galerie** (CRUD + upload), À propos & Commissions (pills internes : sections éditables, mandats avec **photos de membres, comptes liés, import de l'équipe précédente, réordonnancement** · commissions & années promo), **Candidatures** (campagne d'appel à candidatures bureau — titre/intro/postes/open-close, lien de partage à copier — + gestion des candidatures reçues : statut, suppression avec avertissement, export CSV). Badges explicites quand une clé serveur (YouTube/Resend/service_role) manque. Detail: `docs/platform/production-readiness.md`.
* **Recruitment portal:** `/candidature` — standalone link-only application form (Google Form replacement): renders **without site chrome** (no navbar/footer, no path into the main website), `noindex`, anonymous or signed-in submissions (members pre-filled + profile auto-linked), honeypot anti-bot + DB-level anti-abuse (one application per name+phone per campaign, 300/hour cap, forged status/`profile_id` rejected by policy); submissions land in the console "Candidatures" tab, PII readable by bureau only (RLS).
...
* **Resilience & security:** every public page falls back to the static `src/data` seeds when the DB is unreachable; RLS is the enforcement layer (contact info bureau-only via security-definer RPCs; hardening v2.4: candidature spam/dedup caps + forged-status block, privesc guard on self-profiles, banned members excluded from annuaire/emails, SVG uploads rejected, vote attribution hidden, admin-on-admin reset/delete blocked, HSTS); `events_visible` masque réellement la section (redirect + nav + sitemap); forgot-password flow (email reset link); optional Turnstile captcha + Resend email broadcast (dormant until configured — re-send guard ready).
...
* **Engineering:** Next.js **15.5** + React 19 (npm audit clean), CI on GitHub Actions (tsc/lint/vitest/build), 22 vitest unit tests, weekly pg_dump backups to Actions artifacts.
* **Status:** live but **walled pre-launch** — every path except `/candidature` requires a bureau/admin session, and anonymous REST reads of content are RLS-blocked too (`site_is_open()` gates the public policies); the console toggle « Accès public du site » (`site_wall_open`) opens pages **and** data together on launch day. DB schema **v2.5** applied (v2: FK indexes, RLS init-plan fix, RSVP cache, realtime, gallery · v2.2: mandat photos/links · v2.3: candidatures · v2.4/v2.4.1: security hardening — applications anti-abuse, two-layer privesc guard, broadcast idempotency, votes privacy, HSTS · v2.5: content lock behind the wall). Console production sweep + pre-launch security audit (2026-08-31/09-01, independent review: no critical) — see `docs/platform/production-readiness.md`.
...
│   │   ├── production-readiness.md # 2026-08/09 console sweep: mandats, accounts, home content
...
├── next.config.mjs             # Server-mode config (static export retired 2026-08-25; images unoptimized)
